...
d3 = 1 # size of Kohonen layer 3
d5 = d2 = C = 2 # y and y' output layer 

# initialize parameters randomly
U_13     = 0.01*np.random.randn(d1, d3) #2xZ_layers
V_53     = 0.01*np.random.randn(d5, d3) #2xZ_layers
W_32     = 0.01*np.random.randn(d3, d2) #Z_layers x2
W_hat_34 = 0.01*np.random.randn(d3, d4) #Z_layers x2
...
for t in range(epoch):
    ...
    for i in range(Train_samples.shape[1]):
        ...
        alpha_X_dash = 0.5*alpha_X_dash
        beta_Y_dash = 0.5*alpha_X_dash    
    ...
    for i in range(Train_samples.shape[1]):
        ...
        w_Y_dash = 0.5*w_Y_dash
        w_X_dash = 0.5*w_Y_dash
